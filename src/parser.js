...
function parseXML(xml) {
  ...
  while ((m = tradeRegex.exec(xml)) !== null) {
    ...
    // Skip non-USD trades (Canadian stocks etc)
    if (attrs.currency && attrs.currency !== 'USD') continue;
...
function parseCSV(csv) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (iCurr >= 0 && cols[iCurr] && cols[iCurr] !== 'USD') continue;
...
function buildTrades(executions) {
  ...
  for (const [key, execs] of Object.entries(bySymDate)) {
    ...
    // Sort chronologically by full timestamp including seconds
    execs.sort((a, b) => (a.timeFull || a.time || '').localeCompare(b.timeFull || b.time || ''));

    // Merge partial fills: same side + same second → one execution with weighted avg price
    const merged = [];
    ...
    // Position tracker — open lots track { price, qty, comm, time }
    let position = 0;
    let openLots = [];  // FIFO queue of buy lots (or sell lots for shorts)
    let totalOpenComm = 0; // running total of entry commission for current position
    let totalOpenQty = 0;  // total qty in current position
    let side = 'Long';
    ...
    const closeTrade = (exitPrice, exitQty, exitComm, exitTime) => {
      ...
      for (const lot of openLots) {
        ...
        matched.push({ price: lot.price, qty: take, time: lot.time });
        lot.qty -= take;
        remaining -= take;
      }
      openLots = openLots.filter(l => l.qty > 0);
      if (!matched.length) return;
      const matchedQty = matched.reduce((a, b) => a + b.qty, 0);
      const avgEntry = matched.reduce((a, b) => a + b.price * b.qty, 0) / matchedQty;
      const entryTime = matched[0].time || '';

      // Pro-rate entry commission based on how much of the position we're closing
      const entryCommShare = totalOpenQty > 0 ? totalOpenComm * (matchedQty / totalOpenQty) : 0;

      trades.push({
        date, sym, side,
        time: entryTime,
        exitTime: exitTime || '',
        shares: Math.round(matchedQty),
        entry: round2(avgEntry),
        exit: round2(exitPrice),
        comm: round2(entryCommShare + exitComm),
    ...
    for (const ex of merged) {
      ...
      if (position === 0) {
        ...
        openLots = [{ price: ex.price, qty: ex.qty, time: ex.time }];
        totalOpenComm = ex.comm;
        totalOpenQty = ex.qty;
        position = isBuy ? ex.qty : -ex.qty;

      } else if (position > 0 && isBuy) {
        // Add to long
        openLots.push({ price: ex.price, qty: ex.qty, time: ex.time });
        totalOpenComm += ex.comm;
        totalOpenQty += ex.qty;
        position += ex.qty;
        ...
        if (Math.abs(position) < 0.001) {
          position = 0; openLots = []; totalOpenComm = 0; totalOpenQty = 0;
        } else if (position < 0) {
          // Flipped to short
          side = 'Short';
          const flipQty = Math.abs(position);
          openLots = [{ price: ex.price, qty: flipQty, time: ex.time }];
          totalOpenComm = 0; totalOpenQty = flipQty;
        }
    ...
    // Close any remaining open position at last known price
...
function normalizeTimeFull(raw) {
  ...
  const t = parts[1].trim();
  if (!t) return '';
  if (t.includes(':')) return t.slice(0, 8);
